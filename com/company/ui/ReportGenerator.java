...
public class ReportGenerator {
    ...
    public static JPanel createReportPanel(String currentRole, int currentUserId) {
        ...
        filterCard.add(createStyledLabel("Student Roll:", NORMAL_FONT), gbc);
        gbc.gridx = 1;
    ...
    private static void generateReport(DefaultTableModel model, String roll, String subject,
        ...
        try (Connection conn = DatabaseManager.getConnection()) {
            ...
            sql.append("AND s.session_date BETWEEN ? AND ? ");
            sql.append("ORDER BY s.session_date DESC");
            
            try (PreparedStatement ps = conn.prepareStatement(sql.toString())) {
                int paramIndex = 1;
                if (!roll.isEmpty()) ps.setString(paramIndex++, roll);
                if (subject != null && !subject.isEmpty()) ps.setString(paramIndex++, subject);
                if (!"All".equals(status)) ps.setString(paramIndex++, status);
